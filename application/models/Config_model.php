...
class Config_model extends CI_Model {
    ...
    public function getBuses($where = NULL) {
        $this->db->select('*');
        $this->db->from(self::BUSES_TABLE);
    ...
    public function getCountry($where = NULL) {
        $this->db->select('*');
        $this->db->from(self::COUNTRY);
        if ($where !== NULL) {
            if (is_array($where)) {
                foreach ($where as $field=>$value) {
                    $this->db->where($field, $value);
                ...
                $this->db->where('country_id', $where);
            }
        }
        $result = $this->db->get()->result();
        if ($result) {
    ...
    public function insertBooking(Array $data) {
        if ($this->db->insert(self::BOOKING, $data)) {
            return $this->db->insert_id();
        } else {
            return false;
        }
